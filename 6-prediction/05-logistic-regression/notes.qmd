...
email <- email |>
  mutate(y_hat_m1 = 
           ifelse(test = p_hat_m1 > .5, yes = 1, no = 0))

```

Finally, we can calculate $\text{MCR}$. First, we can find all of the emails for which the `y` and `y_hat_m1` don't match. Then, we can take the *proportion* of those e-mails out of all e-mails in the dataset.
