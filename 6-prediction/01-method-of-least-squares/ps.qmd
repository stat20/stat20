...
$$ \text{RSS} = \sum_{i = 1}^n \left(y_i - (b_{0} + b_1x    )\right)^2$$
```{r}
#| echo: true
#| eval: false
rss <- function(coef) {
...
$$ \sum_{i = 1}^n \left|y_i - (b_{0} + b_1x)    \right|$$
...
@. Use `rabs` to estimate the parameters $b_0$ and $b_1$ using `optim()` as you did before. *Play around with starting values for the `par` vector until you get something that you like*; **do not plug in the exact estimates you get from using `lm()`.** Fill in the template below with the starting values that you settle on. 
