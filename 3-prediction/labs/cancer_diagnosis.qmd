...
The purpose of this lab is to practice using the code from the [classification case study notes](https://www.stat20.org/3-prediction/16-classification-case-study/notes.html). **Much of parts 2 and 3 can be answered by copying/pasting/tweaking the code from these notes.**
...
cells <- read_csv('brca_fna_data.csv')
...
6.  *Split the full cells dataset into an 80/20 train/test set split. Name the training data frame `train_df` and the test dataframe `test_df`.*
