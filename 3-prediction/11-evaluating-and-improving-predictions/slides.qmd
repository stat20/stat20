...
q2 <- mpg %>% filter(class %in% car_type) %>%
    mutate(class = factor(class, levels = c("midsize", "suv")))

# Obtain fit
fit <- lm(hwy ~ cty + class, data = q2)
...
# Generate the plot.
ggplot(q2) + 
    geom_line(mapping = aes(x = cty, y = .fitted, color = class)) +
    ylim(c(20,28)) + xlim(16,19) + 
    labs(color = "Class",
         x = "City MPG",
         y = "Highway MPG",
         title = "City MPG exhibits a positive relationship with Highway MPG") + 
