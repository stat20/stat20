...
-  Gmail's spam detector deciding whether or not a stat20 announcement is spam
...
These are all examples of predictive modeling.
...
> Classification is predictive modeling when the response variable is categorical.
...
![](images/lter_penguins.png){fig-align=center width="400"}
...
![](images/bill_length.png){height="200"}

![](images/flipper_length.png){height="200"}
...
<!-- ![](images/culmen-depth.png){fig-align=center width="400"} -->
...
Lets try a simple data driven classification rule; classify a penguin's species based on the species of the most similar penguin in the historical (training) dataset. 
In other words, the use following algorithm to guess the species a new penguin:
...
In math, we would find the smallest value of 
$$
...
If $d=1$ this reduces to the absolute difference i.e. $d(a, b) = |a - b|$. 
If the vectors represent spatial coordinates then Euclidean distance is the normal spatial distance we are familiar with i.e. what you would get if you use a ruler to measure distance between two points on a piece of paper.
...
In other words, we find the penguin in the historical dataset whose Euclidean distance of bill/flipper length is nearest the test penguin.
...
Let say our test penguin has a bill length of 50 mm and flipper length of 200 mm.
...
We compute the Euclidean distance between the test penguin and each penguin in the dataset (plot below), then find the nearest penguin.
...
penguins %>% 
    ...
    geom_point(x=test_flipper_length, y=test_bill_length,
               shape='X', size=10, color='black') + 
    geom_segment(aes(xend = flipper_length_mm, yend = bill_length_mm), x=test_flipper_length, y=test_bill_length, color='red', alpha=.1)+
    theme_bw() + 
...
penguins %>%
    mutate(euclid_dist=sqrt((bill_length_mm - test_bill_length)^2 + (flipper_length_mm - test_flipper_length)^2)) %>% 
    arrange(euclid_dist)
```
...
One thing should seem weird about this code; `test_bill_length` and `test_flipper_length` are numbers but `bill_length_mm` and `flipper_length_mm` are vectors (columns of the penguins dataframe). 
Why can you use numbers and vectors in the same arithmetic expression?
For example `bill_length_mm - test_bill_length`.
...
Looking carefully at the below plot we see the nearest penguin is a Gentoo, but the other nearby penguins are all Chinstraps.
...
penguins %>%
    mutate(euclid_dist=sqrt((bill_length_mm - test_bill_length_2)^2 + (flipper_length_mm - test_flipper_length_2)^2)) %>% 
    arrange(euclid_dist)
```
...
These notes presented classification; predictive modeling when the outcome variable is categorical instead of continuous.
...
Start with training dataset $x_{1}, \dots, x_n$ with labels $y_1, \dots, y_n$ and pick a value of $K$ less than or equal to $n$.
