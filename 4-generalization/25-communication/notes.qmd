...
For the results our our data analysis to matter we have to communicate them to someone else.
...
[^info]: https://en.wikipedia.org/wiki/Information_theory
...
[^businesses]: Businesses are not customer-minded (e.g. "the customer is always right") out of the goodness of their hearts; adaptive to their audience will earn them more money.
...
Many people in your audience will only skim or pay half attention to what you are saying. Structuring your communication with these constraints in mind will go a long way (e.g. provide critical context, make the most important messages easy to see with little thought).
...
[^message]: https://www.storytellingwithdata.com/blog/2017/3/22/so-what This blog post is worth reading in full.
...
> How can I know what I think until I see what I say.[^anderson]

[^anderson]: High-school English teacher Richard Anderson.
...
Textbooks are organized into: chapters, sections, subsections, paragraphs, sentences etc. Imagine if a textbook were just a straight wall of text! We categorize living creatures into a hierarchical taxonomy (kingdom, phylum, ...). Items in grocery stores are group together in aisles/sections and the aisles are roughly arranged to be similar (e.g. the fruit section is near the vegetable section). The [topics in stat20](https://www.stat20.org/schedule.html) are grouped together into units (e.g. questions, prediction, generalization). The [taxonomy of data](https://www.stat20.org/1-questions-and-data/02-taxonomy-of-data/notes.html#a-taxonomy-of-data) gives a hierachical organization of data types.
...
To illustrate these two types of plots lets use a dataset from Capical Bikeshare.[^capital]
Capital Bikeshare is a bike sharing system for Washington DC that makes their hourly rental data publicly available.

[^capital]: See https://capitalbikeshare.com/how-it-works. They make their data publically available at https://ride.capitalbikeshare.com/system-data.

```{r}
library(tidyverse)
bikes <- read_csv('https://raw.githubusercontent.com/idc9/stor390/master/data/bikes_2011.csv')
bikes %>% 
...
The target audience for an exploratory plot is the person making it and possibly close collaborators. It is created quickly (e.g. 2 lines of code) and contains lots of information (e.g. every data point).


Let's take a look at the rental counts per hour over the course of the year for Captal Bikeshare (note working day = Mon-Fri vs. Weekend/Holiday).
...
- Use median count per hour as a summary (i.e. we don't show every data point).
...
- Use line **breaks an indentation**. For example, compare the following two snippets that do the same thing
...
Many people follow [Hadley's style guide](http://adv-r.had.co.nz/Style.html) for R code.
...
![Bad question](images/bad_question.png)

![Good quesiton](images/good_question.png)
:::
...
![](images/duck.png){fig-align=center width="400"}
<!-- Duck from https://www.nytimes.com/2011/11/13/magazine/the-rubber-duck-knows-no-frontiers.html -->
 

Often the act of simply asking a good question will cause you to figure out the problem. By simply trying to explain what's going on can cause you to see the issue. This called *rubber duck debugging*[^duck] and can be an effective strategy to debug your code; pick up a rubber duck and explain your problems.[^email] 
