...
Recall the case study we saw about donkeys in Kenya. The question in the study was how to predict the weight of donkeys using measurements that were easier to obtain. To build their model, the scientists obtained data by measuring a sample of 25 donkeys at each of 17 sites in 2010[^donkey].  Four body measurements were taken on each donkey: liveweight (kg), heart girth (cm), height (cm), and length (cm), and in addition, sex and body condition score were recorded (the bcs categorized the donkeys according to their condition, ranging from "emaciated" to "obese"). (Note that girth or heart girth  is the circumference of the body, measured just behind the front legs.)

Set up a box model for this process (sampling donkeys), where you want to compute the average weight and average girth for all of the donkeys sampled from the 17 sites. You might not be able to completely specify all 4 parts of the process. 

<details><summary>Check your answer</summary>

We have 17 boxes, one for each site. In each box, there is one ticket for each donkey that visited the site. We were not given the information as to how many donkeys visited each site so we can't specify the number of tickets in the box. The ticket has information from all 8 variables recorded (3 categorical and 5 numerical). 
We draw 25 tickets from each box at random *without* replacement.
We compute compute two averages from the draws across all 17 boxes.
...
You have seen the penguins data, which measured `r ncol(penguins)` variables on a sample of `r nrow(penguins)`. 
The researchers chose pairs of penguins that had nests with one egg in them. Let's assume that the selection process was at random from all in the archipelago. (The details of the selection process are not given in the research paper, so for this example, we have made the assumption that a chance process was used).
Suppose we measured sex, species, and flipper length, to compare the flipper lengths of males and females, grouped by species.
Set up the box model for this process.

<details><summary>Check your answer</summary>

The box would have an unknown number of tickets, one for each penguin. One ticket for each nest in the archipelago that had one egg in it. Each ticket would have 6 pieces of information on it: the species, sex, and flipper length in mm of the pair of penguins.
We would draw 172 tickets at random without replacement, and that would give us a sample of 344 penguins. Each ticket would have 3 things written on it: the species, sex, and flipper length in mm of the penguin.
We would draw 50 tickets at random without replacement, and 
![](images/penguin-ticket.jpeg){fig-align=center width="200"}
