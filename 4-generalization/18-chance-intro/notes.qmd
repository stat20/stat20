...
subtitle: "We will introduce the ideas of probability via a mechanism called a box model "
...
#### Example 6: Selecting an apple from a bag of fruit
...
#### Example 7: Penguins
![](images/committee.jpeg){fig-align=center width="300"}

You have seen the penguins data, which measured `r ncol(penguins)` variables on a sample of `r nrow(penguins)`. Here the box represents the penguin population of  three species: Adelie, Chinstrap, and Gentoo. That is, each ticket in the box represents one penguin.There should be as many tickets in the box as there are Adelie, Chinstrap, and Gentoo penguins combined.[^npenguins]


[^npenguins]: An article from February claims that there are 10 million Adelie penguins, 8 million Chinstrap, and 774,000 Gentoo penguins on the Antarctic continent.(https://polarguidebook.com/why-are-penguins-endangered/)

#### Example 8: Selecting 2 people out of a group of 5 
...
What is the probability that Alex and Max will be selected? Guess! (Hint: you have seen all the possible pairs above, and they are equally likely. What will be the chance of any one of them?)
...
We can use `sample()` to *estimate* the chance of a particular ticket being drawn when we aren't sure of what that chance might be. We would do this by repeatedly sampling from the "box" with replacement (many times), then computing the proportion of times we drew each ticket. For example, say we consider the box from above, and want to estimate the chance of each ticket.

In the code below, another new functions are introduced: `replicate()`. The function `replicate(reps, expr)` is a very useful function that takes as input an expression `expr` and evaluates it `reps` times, returning a vector. 
...
ggplot(as.data.frame(draws), aes(x=draws)) + 
  ...
  xlab("ticket drawn")
```
We see that the chance of drawing a $\fbox{2}$ is about 0.4, twice the chance of drawing any other ticket, which makes sense as there were two tickets marked 2.

#### Tossing a fair coin

We can estimate the probabilities of various outcomes related to coin tossing, using the box. 
...
We know all the possible outcomes of summing a pair of dice (between $2$ and $12$ spots). Why not make a box with tickets labeled $2, 3, 4, \ldots, 12$ and draw *once* from that box? 
:::


## Rules of chance

What we have done here is estimate probabilities using the idea that the *chance* or *probability* of something is the proportion of times it occurs when we simulate over and over again the basic process that results in the thing we are counting. This is using the definition of the chance of an outcome to be the proportion of times we expect it to occur when the process that generates the event is  *independently* repeated many times [^freq].
...
2. The chances of all possible things that might happen as a result of some action will add up to 1. 
...
[^uar]: We call the tickets equally likely when each ticket has the same chance of being drawn. That is, if there are $n$ tickets in the box, each has a chance of $1/n$ to be drawn. We also refer to this as drawing a ticket *uniformly at random*, because the probabilities of drawing the tickets are the same, or *uniform*.