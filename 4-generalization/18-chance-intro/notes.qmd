...
#### Example 1: Drawing a ticket from a box 
...
We could compute a weighted average of the possible draws, weighted by their chance. We would get: $1\times \frac{1}{5} + 2 \times \frac{2}{5} + 3 \times \frac{1}{5} + 4 \times \frac{1}{5} = 2.4$. Note that the average of the tickets in the box is the same.



How about if we draw 100 times with replacement from this box and see what our sample looks like (we will explore the code later in the notes). That is, we will see what proportion of the draws are 1's, 2's etc.
...
We can see that the *sample proportions* look similar to the *population (box) proportions*, but are somewhat different. In fact, it turns out that the counts of drawn ticket are:
...
The purpose of creating a box model is to understand the variability in a *chance process* such as tossing a coin over and over, and counting the number of heads, or sampling voters to ask them if they will vote for Prop 1 in November's elections. We do this by having the tickets represent the outcomes in the chance process, and using the variability of the tickets drawn to understand the variability in the outcomes of the chance process.
...
We assume that the tickets in the box are *equally likely* to be drawn.

#### Example 2: Tossing a fair coin once 
...
#### Example 7: Selecting 50 penguins from the penguins dataset.
