...
2. Pre/post design
3. Stability of outcomes across time
4. Time trend, interrupted time series (ITS)
5. Linear trend assumption
6. Pure control, difference-in-difference (DiD) design 
7. Parallel trend assumption
8. Dependence across repeated observations

#### Tool Acquisition

1. Visualize panel data using `geom_line()`
2. Check data (how many repeated measures? pure controls?) to determine whether ITS or DiD can even be fit and filter if needed
3. Specify regression model to estimate ITS effect
4. Compute DiD effects using `calculate()` 
5. Use `pivot_wider()` to construct pre/post differences



#### Concept Application

1. Evaluate stability/linearity/parallel trends assumptions using line plots
2. Choose between matching and panel methods when pure controls are present
3. Describe pre/post, ITS, and DiD effects graphically on plots with average trend lines for study groups.
4. Bootstrap pair differences for confidence intervals and hypothesis tests in pre/post and DiD designs
...
The dashed line represents a counterfactual average outcome for the treated units if they had not received treatment, and we are interested in learning the difference between this dashed line and the actual outcome for the intervention group. Although we don't have multiple observations prior to treatment to estimate the slope of the dashed line anymore, we can learn it by looking at the pre/post outcome differences in the control group below.  We can then get an effect estimate by subtracting the pure control pre/post difference in the outcome $Y$ from the treated group's  pre/post difference in outcome $Y$:
...
checkout_w_control <- checkout |>
  ...
  pivot_wider(id_cols = store_id,
              names_from = time, 
              values_from = c(sales,checkout_policy))
...
To do a hypothesis test or create a confidence interval, we must usually begin by transforming the data frame so each row represents a different unique subject.  The `pivot_wider` command does this, creating new pre- and post- versions of our other variables for each store.  We call the new data frame `checkout_by_store`. This code chunk shows how we can calculate the effect estimates from store-specific sales differences using `infer` commands.
