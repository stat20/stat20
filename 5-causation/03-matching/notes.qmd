...
head(grad_job)
```
We use the \texttt{MatchIt} package in R to find the nearest neighbor for each 
treated unit using the 
Euclidean distance, forming five matched pairs  We can use the \texttt{plot} function to compare how close paired units lie on each variable.
...
The gray lines give the density of the control distribution for each covariate 
and the black lines
give the density of the treated distributions.

We can also make a Love plot comparing the standardized differences before and
...
albeit with a slightly higher average GPA among the Cal grads.
...
Nearest-neighbor matching may not always fix balance this effectively.  Sometimes
