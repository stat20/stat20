...
3. Optimal matching and associated improvements to balance
4. Unobserved confounding
5. Natural experiment
6. Paired permutation tests for matched data

#### Tool Acquisition

1. Create matched datasets via `matchit()` and `match.data()`
2. Check covariate balance after matching
3. Construct pair differences from long data using `pivot_wider()`



#### Concept Application

1. Identifying possible unobserved confounders.
2. Using balance checks to assess whether a study is plausibly a natural experiment
3. Using balance checks to assess whether matching has successfully removed confounding
4. Paired permutation test using `infer`
...
When we do not get to assign treatments randomly, treated and control subjects may
look systematically different even before the treatment takes effect. Let's return to our example of evaluating the impact of graduating from Cal on obtaining a good job.
The treated group of students (who choose to go to Cal) probably look different from the 
control group (who choose not to) even before entering college; for example, the
Cal students may have studied more in high school, and more of them may live in
California.  These covariate imbalances threaten our ability to learn about
the effect of Cal graduation, since treated-control differences in our outcome
(getting a good job) could instead be explained by high school performance or state of
residence.  Since we can't induce covariate balance by choosing who graduates from
Cal, we need another approach to learn about the causal effect.
...
Even in studies where we can obtain very close matches between treated and control subjects, matched observational studies have at least one important weakness compared to randomized trials: they only correct imbalances in the covariates we happened to measure.
 In the matched study of graduation from Cal, for example, we don't have any information about the career goals or interests of these students. Perhaps the Berkeley students all chose to attend because they love statistics and their siblings or matched controls all skipped college and spent their time practicing the guitar because they dream of becoming rock stars; then maybe the real reason the former group is employed is because of their interest in a field with good job opportunities, not because of the college they attended. In the smoking twins study, it could be that the smoking twins pursued other kinds of risky behaviors (e.g. drug use, poor diet, working in dangerous jobs) at a higher rate than the non-smoking twins despite their genetic similarity. In either case, we can't be sure whether any apparent effect we observe is due to our treatment or due to these hidden differences.  <!--Contrast this with a randomized trial: assigning subjects at random will tend to balance *all* covariates, observed and unobserved.-->

The data does not help us much to refute concerns about variables that it does not contain. We have to rely on our judgment about the context to evaluate whether such hidden imbalances are more plausible than the treatment effect. This situation is called unobserved confounding, and if it is present it can lead to substantially biased treatment effect estimates.
...
Some observational studies carry much greater concerns about unobserved confounding than others. In the Cal graduation study, subjects had a high degree of agency about whether they ended up in the treatment or the control group (since nobody is forced to graduate from Cal).  This is usually problematic for determining causal relationships, since people who know they will benefit from treatment may choose to take it, while those who know it will not help them may decide not to: this kind of difference is a major unobserved confounder! 

However, in some observational studies treatment is assigned by a process that cannot easily be influenced by the subjects themselves and may ignore differences between subjects so completely as to be almost random. Causal conclusions drawn from these 
studies, which are sometimes called \emph{natural experiments} or quasi-experiments, are more reliable than those in other observational studies..
...
It is important to notice how to specify the formula inside `matchit`. The treatment variable goes on the left-hand side, the covariates go on the right-hand side, and the outcome variable doesn't appear at all (since it is not used when constructing matched pairs): `treatment ~ covariates`. Compare this to the formulas used for `lm`, which are of form `response ~ predictors`.
...
#### Comparing outcomes in matched pairs

Since after matching our comparison groups closely resemble what we might have seen in a randomized experiment, we report the sample difference in means as our estimate of the average treatment effect.  To use the data only from the matched individuals, rather than everyone in `grad_job`, we start with the output object from the matching step and pipe it to the `match.data` command to obtain the smaller data frame.
...
a good job by 0.4. This seems like a large effect, but we have a small dataset -- are we confident that this is not just a random deviation from zero?  To answer this question we will conduct a hypothesis test using the `infer` package.

Hypothesis testing for matched data differs in one important respect from previous tests we have studied for comparing two groups. Our data now has a special structure in which every subject has a matched counterpart in the other group specially chosen to be similar to itself.  If we permute the entire column of observed outcomes in our data frame to generate data from the null distribution, we will destroy that special structure; in any particular permutation both outcomes from a single pair may end up in the same group. A better idea is to permute outcomes **within** pairs only.  In other words, we decide at random in each pair whether to switch which outcome is for the treated unit and which is for the control unit.  This way any dataset we generate from the null distribution still has similar subjects organized into pairs with one treated unit and one control in each pair. 

To conduct this kind of hypothesis using `infer` we need to do two things.  First, we need to transform the data frame so each row represents a different matched pair, and create a new variable which gives the treated - control difference for this pair. The `pivot_wider` command helps us transform the data frame in this way.  The `id_cols` argument determines what will be a row in the new data frame.; here we pass it the `subclass` factor variable created by `match.data()`, which has a different factor level for each distinct matched pairs, to make sure we get a row for each matched pair. The next two arguments, `names_from` and `values_from` should be the treatment and outcome respectively, and the fourth argument is an optional prefix added to the column names.
```{r}
by_pair <- grad_matched |>
   match.data() |>
   pivot_wider(id_cols = subclass,
               names_from = cal_grad, 
               values_from = good_job, 
               names_prefix = 'good_job_cal_grad') |>
   mutate('treated_control_diff' = 
            (good_job_cal_gradTRUE == 'yes') - 
            (good_job_cal_gradFALSE == 'yes')) 
by_pair
```
Second, we use a different argument to `hypothesize` called `paired independence` to communicate how our pair differences are supposed to be permuted.  Because we have mutated treated and control outcomes into a single new variable (`treated_control_diff`), we don't need to specify an explanatory variable, and our test statistic will just be the mean of `treated_control_diff`.
```{r}
null <- by_pair |>
  specify(response = treated_control_diff) |>
...
obs_stat
```
Notice that we got the same value for our observed difference in means in this code chunk as in the one above that used the `stat = "diff in props"` argument inside `calculate()`;  because
$$
\frac{1}{n}\sum^n_{i=1}x_i - \frac{1}{n}\sum^n_{i=1}y_i = \frac{1}{n}\sum^n_{i=1}(x_i - y_i),
$$
it doesn't matter whether we take group averages or take within-pair group differences first.
...
The p-value suggests that although the observed test statistic may appear large, values this large are fairly common under the null hypothesis that graduating from Cal and obtaining a good job are independent.
