...
In this section, we will introduce a new kind of histogram that is a very natural way of thinking about probabilities, and has useful consequences.
...
- **Arguments** 
    ...
    - `times`: the number of times we should repeat the elements of `x`. This could be a vector the same length as `x` or it could be a single number, in which case the entire `x` is repeated that many times.
    - `each`: the default is 1, and if specified, each element of `x` is repeated `each` times. 
    
**Example: the outcomes from rolling a pair of dice and summing the spots**
