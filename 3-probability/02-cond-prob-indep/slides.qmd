...
On each turn of a game, I first roll a fair six-sided die, and then toss a coin as many times as the number of spots I rolled.

What is the probability that all my tosses land heads on a particular turn?

:::{.notes}

No pollev, as this question is confusing, better to go through it. The probability of coin landing heads on all tosses is P(all H & 1 toss) + P(all H & 2 tosses ) + etc = P(all H | k = 1)*P(k =1) + ... =  (1/2)*(1/6) + (1/4)*(1/6) + ...+ (1/64)*(1/6). The point is using the multiplication rule for dependent events.
:::
...
:::{.notes}
Draw a picture here to show how we can write B like this
:::
...
:::{.notes}
Then review PS 8. If you have time, go over coin flipping code
:::