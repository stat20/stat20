...
If you like you can read the code below by clicking on the arrow, and we will go over the function `replicate()` later in the section "Ideas in Code".
...
Before we get to the ideas in code, let's review a couple of very important ideas. The first has to do with events that are **not** independent!
...
We will go over the code we used to simulate the dice games and introduce the useful function `replicate()`, but before that I want to introduce a very useful file type called an R script files. 

### R script files

These are files with the extension `.r`, so your file might be called `tips-and-tricks.r`, for example. 
