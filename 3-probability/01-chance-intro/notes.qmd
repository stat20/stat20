...
:::{.def}
**P(A)**
 ~ For any event $A$, we write the probability of $A$ as $P(A)$.  To find the probability of an event $A$ when all the outcomes are equally likely, just count how many outcomes are in $A$ and divide this number by $n$, the total number of outcomes in the outcome space $\Omega$.
:::
...
The complement rule
: $P(A) + P(A^C) =  1$ (since $A  \cup A^C = \Omega$, and $A  \cap A^C \emptyset$).
...
Make a table listing the possible outcomes when we draw a ticket from the box, and the probability of each outcome.
...
Let $A$ be the event of drawing an even number, then $A = \{ 2, 4\}$, or if we want to show equally likely outcomes, we would list 2 twice because there are two tickets marked 2, making it twice as likely as any other number ($\{ 2, 2, 4\}$). We see that $P(A) = 3/5$
