...
To reiterate, we all have some notion of chance or probability, and ask questions like:
...
You can see here that the poor Chevalier wasn't as good a mathematician as he imagined himself to be, and didn't compute the chances correctly.  `r emo::ji("monocle")` You can play with this simulation in the ``Ideas in Code'' section below.
...
The histograms we have seen so far have been visualizations of data, and so are called *empirical* histograms. We can represent the probabilities visually with bars, and we call this a *probability histogram*. Note that we do not need to collect data for this type of graph since the heights of the bars represent probabilities, and are *theoretical*.
...
How about if we draw 50 times at random with replacement from this box and see what our sample looks like. That is, we will see what proportion of the draws are 1's, 2's etc.
...
What we have seen here is how when we draw at random, we get a sample that resembles the population, that is, a *representative sample*. This is the *empirical* histogram, that is, a histogram derived from data.
