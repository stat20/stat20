...
#### `sample()`: randomly picks out elements (items) from a vector
...
(Note: The `set.seed()` function below ensures that we will get the same random sample each time we run the code.)[^setseed]
