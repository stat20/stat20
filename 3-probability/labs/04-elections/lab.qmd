...
Inside the `stat20data` package there is a function called `get_first()` that pulls off the first digit of every element in a vector. Piece all four plots together into one single plot using the `patchwork` library (see code in the [notes on Numerical Data](2-summarizing-data/05-communicating-with-graphics/notes.qmd) for an example of its use).
