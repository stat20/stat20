...
: A constant associated with the distribution. If you know the parameters of a probability distribution, then you can compute all the values of $f(x)$.
...
: of a random variable $X$ is defined for *every* real number, and gives, for each $x$, the amount of probability or mass that has been *accumulated* up to (and including) the point $x$, that is, 
$F(x) = P(X \le x)$.
