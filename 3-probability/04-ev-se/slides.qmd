...
What about if we don't want to *count* but want to plot probabilities? 

Let's look at our favorite example, the number of heads when we toss a coin three times. We know that:
...
X = \begin{cases}
		 ...
		 \end{cases}
$$
To plot this, we use a special kind of bar chart, plotted using `geom_col()`, in which the bar heights represent *values* supplied by the data. We have to specify a `y` aesthetic.
