...
This means that the expected value of an average does *not* scale as $n$, but $ E(\overline{X})$ is the *same* as the expected value of a single random variable. Let's check the variance now:
...
Note that, just like the sample sum $S_n$, the sample mean $\displaystyle \overline{X}$ is a random variable, and its variance scales as $\displaystyle \frac{1}{n}$, which implies that $SD(\overline{X})$ will scale as $\displaystyle \frac{1}{\sqrt{n}}$.
