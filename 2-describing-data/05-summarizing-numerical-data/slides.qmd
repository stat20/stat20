...
<center>
<!-- <iframe src="https://embed.polleverywhere.com/multiple_choice_polls/VuDFg0bdr6QKhCwLIu2gt?controls=none&short_poll=true" width="800px" height="600px"></iframe> -->
<iframe src="https://pollev-embeds.com/multiple_choice_polls/VuDFg0bdr6QKhCwLIu2gt/respond" width="800px" height="600px"></iframe>
</center>
...
The most powerful package in R for data wrangling is `dplyr`, and it is loaded when you include `library(tidyverse)`.

1. **`arrange()`**: sorts a data frame according to the values of one (or many) columns.

. . .

2. **`select()`**: selects from a data frame only the columns of interest.

. . .

3. **`mutate()`**: creates a new column in a data frame that can be a function of existing columns.

. . .

> All of the `dplyr` functions take a data frame as their first argument and return a data frame.

## Summarize

. . .

Calculates summary statistics on the columns of a data frame.

```{r}
#| eval: false
#| echo: true
#| error: true
summarize(DATAFRAME, NAME = FUN1(COL1))
```

- `FUN1` can be any function that can operate on a vector (like `mean()`).
- `COL1` is the name of a column found in `DATAFRAME`.
- You can optionally call the resulting statistic a `NAME` of your choice.
