...
d <- purrr::map_chr(materials_list, "date") |>
  lubridate::mdy()

library(lubridate)

first_day_of_class <- min(d)

first_mon <- first_day_of_class - (wday(first_day_of_class) - 1)

first_week <- first_mon + 0:4
cal <- mday(first_week)

last_day_of_class <- min(d)

lessons <- c("Graphics", "Stats", "Homework", "Models", "Summarizing Categorical Data")
cal_df <- as.data.frame(matrix(c(cal, lessons), ncol = 5, byrow = TRUE))
names(cal_df) <- c("MON", "TUE", "WED", "THU", "FRI")

is_date <- c(TRUE, FALSE)

library(gt)
gt(cal_df) |>
  cols_align(align = "center") |>
  cols_width(everything() ~ pct(20)) |>
  tab_style( # style header
