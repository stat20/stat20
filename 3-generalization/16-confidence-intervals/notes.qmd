...
2. The SD of the population and the SE of the sample averages are related in the following[^finite]:
...
The confidence interval provides an interval estimate, instead a point estimate, that is based on spread of the sampling distribution of the statistic.
...
For the particular interval that you have created, you don't know if it contains the population mean or not. This is why we use the term *confidence* to describe it instead of probability. Probability comes into play when taking the sample, after that our confidence interval is an known observed value with nothing left to chance.
...
$$\left[\hat{p} - 1.96 \frac{\sqrt{p(1-p)}}{\sqrt{n}}, \hat{p} + 1.96 \frac{\sqrt{p(1-p)}}{\sqrt{n}}\right]$$
